import 'package:flutter/material.dart';

class UiScale {
  UiScale._();
  static const double s4 = 4.0;
  static const double s8 = 8.0;
  static const double s12 = 12.0;
  static const double s16 = 16.0;
  static const double s20 = 20.0;
  static const double s24 = 24.0;
  static const double s32 = 32.0;
  static const double s40 = 40.0;
  static const double s48 = 48.0;
  static const double s56 = 56.0;
  static const double s64 = 64.0;
  static const double s72 = 72.0;
  static const double s80 = 80.0;
  static const double s88 = 88.0;
  static const double s96 = 96.0;
  static const double s104 = 104.0;
  static const double s112 = 112.0;
  static const double s120 = 120.0;
  static const double s128 = 128.0;
  static const double s136 = 136.0;
  static const double s144 = 144.0;
  static const double s152 = 152.0;
  static const double s160 = 160.0;
  static const double s168 = 168.0;
  static const double s176 = 176.0;
  static const double s184 = 184.0;
  static const double s192 = 192.0;
  static const double s200 = 200.0;
  static const double s208 = 208.0;
  static const double s216 = 216.0;
  static const double s224 = 224.0;
  static const double s232 = 232.0;
  static const double s240 = 240.0;
  static const double s248 = 248.0;
  static const double s256 = 256.0;
  static const double s264 = 264.0;
  static const double s272 = 272.0;
  static const double s280 = 280.0;
  static const double s288 = 288.0;
  static const double s296 = 296.0;
  static const double s304 = 304.0;
  static const double s312 = 312.0;
  static const double s320 = 320.0;
  static const double s328 = 328.0;
  static const double s336 = 336.0;
  static const double s344 = 344.0;
  static const double s352 = 352.0;
  static const double s360 = 360.0;
  static const double s368 = 368.0;
  static const double s376 = 376.0;
  static const double s384 = 384.0;
  static const double s392 = 392.0;
  static const double s400 = 400.0;
  static const double s408 = 408.0;
  static const double s416 = 416.0;
  static const double s424 = 424.0;
  static const double s432 = 432.0;
  static const double s440 = 440.0;
  static const double s448 = 448.0;
  static const double s456 = 456.0;
  static const double s464 = 464.0;
  static const double s472 = 472.0;
  static const double s480 = 480.0;
  static const double s488 = 488.0;
  static const double s496 = 496.0;
  static const double s504 = 504.0;
  static const double s512 = 512.0;
  static const double s520 = 520.0;
  static const double s528 = 528.0;
  static const double s536 = 536.0;
  static const double s544 = 544.0;
  static const double s552 = 552.0;
  static const double s560 = 560.0;
  static const double s568 = 568.0;
  static const double s576 = 576.0;
  static const double s584 = 584.0;
  static const double s592 = 592.0;
  static const double s600 = 600.0;

  static double scaleSize(BoxConstraints constraints, double baseFontSize) {
    const double minFontSize = 10.0;
    const double referenceWidth = 1200.0;

    double scaleFactor = constraints.maxWidth / referenceWidth;
    double responsiveFontSize = baseFontSize * scaleFactor;

    return responsiveFontSize.clamp(minFontSize, baseFontSize);
  }
}
